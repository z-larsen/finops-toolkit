...
Use it when the user wants to enforce or audit tagging, restrict what can be deployed, or asks for "a policy" to back up a FinOps recommendation. Run `run_scan` with `policy_inventory` and `run_scan` with `policy_recommendations` from the `finops-multitool` MCP server first to see what's already assigned and where the gaps are, then generate policy here.

## Workflow

1. **Inventory** — `run_scan` with `policy_inventory` (existing assignments, scopes, effects, compliance).
2. **Gap analysis** — `run_scan` with `policy_recommendations` (missing tagging/region/SKU guardrails).
